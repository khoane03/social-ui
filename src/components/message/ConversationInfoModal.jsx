import { X, Users, Plus, BadgeCheck, Edit2, Check, Camera, LogOut } from "lucide-react";
import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";

// Mock context
const useAuth = () => ({ user: { id: 1 } });
const useAlerts = () => ({ addAlert: (alert) => console.log(alert) });
...
export const ConversationInfoModal = ({ isOpen, onClose, conversation }) => {
  ...
  const handleToggleAdmin = async (memberId) => {
    try {
      ...
      addAlert({ type: "error", message: error?.message || "Lỗi cập nhật admin." });
    }
  };
  ...
  const handleSaveGroupInfo = async () => {
    try {
      ...
      addAlert({ type: "error", message: error?.message || "Lỗi cập nhật nhóm." });
    }
  };
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      ...
      addAlert({ type: "error", message: error?.message || "Lỗi khi rời nhóm." });
    }
  };
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <img
                            src={m.avatarUrl || "/api/placeholder/40/40"}
                            alt={m.fullName}
                      ...
                      {user.id === conversation?.ownerId && (
                        <motion.button
                          ...
                          className="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors opacity-0 group-hover:opacity-100 bg-gray-100 dark:bg-zinc-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-zinc-600"
                        >
                          {m.isAdmin ? "Gỡ admin" : "Thêm admin"}
                        </motion.button>
                      )}
                    </motion.li>
                  ))}
